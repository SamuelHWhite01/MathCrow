...
function ScoreBoard(){
    ...
    const { setDivisionProblem, divisionProblem } = useDivisionProblemContext();

    const expandedGrid = useMemo(() => {
    const size = userData.divisionTableData.historyGrid.length; // expected 12
    ...
    for (let i = 0; i < size; i++) {
      const newRow = [i + 1, ... userData.divisionTableData.historyGrid[i]];
      expandedGrid.push(newRow);
    }

    return expandedGrid; // 13 x 13 grid with headers
    }, [userData.divisionTableData.historyGrid]);
  
    const getColor = (numAnswered:number ) => {
        const baseColor = [200,200,200]
        const maxCorrect = userData.divisionTableData.maxCorrect
        let intensity:number = 0
        if(maxCorrect !== 0)
        {
            intensity = numAnswered / userData.divisionTableData.maxCorrect //a percentage to take the color from the base color ro the maxColor
        }
    ...
    {
        divisionProblem.nextSelect(f1,f2)
    ...
    return (
        <div>
            ...
            <div className="mt-2">
                {userData.divisionTableData.historyGrid.map((row, i) => (
                    ...
                    onClick={handleExpand}>
                        <div className="flex flex-col gap-4 mr-auto ml-auto w-fit">
                            ...
                            <div key={i} className="h-auto flex flex-row justify-center">
                                ...
                                const numAnswered = userData.divisionTableData.historyGrid[i - 1][j - 1];
                                return (
                                    <button
                                        key={`${i}-${j}`}
